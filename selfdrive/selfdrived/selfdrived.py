...
class SelfdriveD:
  def __init__(self, CP=None):
    ...
    # Longitudinal 1.0: E2E is now the default for all vehicles with openpilot longitudinal control
    # ExperimentalMode param is deprecated but kept for UI backward compatibility
    if not self.CP.openpilotLongitudinalControl:
      self.params.remove("ExperimentalMode")

    # E2E Phase 2: Experimental Mode is now the default behavior (Longitudinal 1.0)
    # All vehicles with openpilot longitudinal control use E2E vision-based planning
    if (self.CP.openpilotLongitudinalControl and
        not self.CP.alphaLongitudinalAvailable):
      # Auto-enable Experimental Mode for backward compatibility with UI
      if not self.params.get_bool("ExperimentalMode"):
        cloudlog.info("Longitudinal 1.0: E2E is now the default for all vehicles")
  ...
  def params_thread(self, evt):
    while not evt.is_set():
      ...
      self.is_ldw_enabled = self.params.get_bool("IsLdwEnabled")
      self.disengage_on_accelerator = self.params.get_bool("DisengageOnAccelerator")
      # Longitudinal 1.0: E2E is always enabled for vehicles with openpilot longitudinal control
      # ExperimentalMode param is kept for UI backward compatibility
      self.experimental_mode = self.CP.openpilotLongitudinalControl
      self.personality = self.params.get("LongitudinalPersonality", return_default=True)
      time.sleep(0.1)
