...
from tinygrad.helpers import DEBUG, getenv, Context
  ...
  def print_report(self):
    ...
    for name, times in self._profiles.items():
      ...
      if stats:
        ...
        if stats['mean'] <= 40.0:
          print(f"  ✓ Meets 20Hz target")
        else:
          print(f"  ✗ Exceeds 20Hz budget")
...
def setup_tici_environment():
  ...
  if DEBUG >= 1:
    ...
    print(f"  Schedule cache enabled")
    print(f"  Kernel fusion enabled")
