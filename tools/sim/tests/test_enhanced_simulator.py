...
class TestEnhancedSimulatorConfig:
  ...
  def test_photorealistic_mode(self):
    ...
    assert bridge.should_render == True
  ...
  def test_randomize_map(self):
    ...
    from openpilot.tools.sim.bridge.metadrive.metadrive_bridge import MetaDriveBridge, create_map
    ...
    assert len(set(len(c['config']) for c in configs)) >= 1  # May occasionally be same
  ...
  def test_spawn_world_with_enhanced_config(self):
    ...
    assert config_template['use_render'] == True
...
class TestSimulatorCLI:
  ...
  def test_parse_args_enhanced(self):
    ...
    assert args.dual_camera == True
    assert args.photorealistic == True
    assert args.weather == 'rain'
    assert args.traffic_density == 0.5
    assert args.randomize_map == True
  ...
  def test_parse_args_defaults(self):
    ...
    assert args.photorealistic == False
    assert args.weather is None
    assert args.traffic_density == 0.0
    assert args.randomize_map == False
